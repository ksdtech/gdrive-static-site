...
MULTISTE Configuration
======================
If you include a MULTISITE global in your settings file, with keys as subsite names,
you can use it to override Pelican's global settings by running Pelican with an
enviroment variable named PELICAN_SITE having the value of the subsite name you 
wish to generate.  Example:
...
Note the use of the YAML_PATHS setting that can be used to parse the metadata for all sites
if you are only generating a single subsite.  We use this to fix links between
the subsites. If YAML_PATHS is not supplied the YamlGenerator will fall back to PAGE_PATHS.

To generate the "kent" subsite in the above example you would invoke Pelican like this:

    PELICAN_SITE=kent pelican -s pelican/pelicanconf.py -t pelican/themes/notmyidea 


Serving URLs Without .html Extension
====================================
See the *_SAVE_AS and *_URL settings, which strip the .html from the URLs.

Then add these Apache mod_rewrite rules:

    RewriteEngine on

    # Serves file.html for domain.com/file
